...
void PdfRenderer::renderImpl()
{
    ...
    try {
#ifdef Q_OS_UNIX
        pdfData.m_fontMgr = SkFontMgr_New_FontConfig(nullptr, std::move(SkFontScanner_Make_FreeType()));
#endif
        ...
        pdfData.m_unicode = SkUnicodes::ICU::Make();
        pdfData.m_shaper = std::move(SkShapers::HB::ShapeDontWrapOrReorder(pdfData.m_unicode, pdfData.m_fontMgr));

        const int itemsCount = m_doc->items().size();

        Q_EMIT progress(0);
        Q_EMIT status(tr("Rendering PDF..."));
                    ...
                    if (!p->items().empty() && p->items().front()->type() == MD::ItemType::Text) {
                        ...
                        if (!t->opts()) {
                            ...
                            if (highlight) {
                                ...
                                auto np = QSharedPointer<MD::Paragraph>::create();
                                ...
                                np->appendItem(i);
                                auto nt = QSharedPointer<MD::Text>::create();
                                nt->setText(text);
        ...
        for (auto it = m_doc->items().cbegin(), last = m_doc->items().cend(); it != last; ++it) {
            ...
            case MD::ItemType::Anchor:
                pdfData.m_anchors.push_back(static_cast<MD::Anchor *>(it->get())->label());
        ...
        for (auto it = m_doc->items().cbegin(), last = m_doc->items().cend(); it != last; ++it) {
            ...
            case MD::ItemType::Paragraph:
                drawParagraph(pdfData,
                              static_cast<MD::Paragraph *>(it->get()),
                              m_doc,
                              0.0,
                              true,
                              CalcHeightOpt::Unknown,
            ...
            case MD::ItemType::Blockquote:
                drawBlockquote(pdfData,
                               static_cast<MD::Blockquote *>(it->get()),
                               m_doc,
                               0.0,
                               CalcHeightOpt::Unknown,
                               1.0,
            ...
            case MD::ItemType::List: {
                auto *list = static_cast<MD::List *>(it->get());
            ...
            case MD::ItemType::Anchor: {
                ...
                SkPoint targetPoint = SkPoint::Make(pdfData.m_layout.margins().m_left, pdfData.m_layout.topY());
                sk_sp<SkData> pageName(SkData::MakeWithCString(a->label().toUtf8().data()));
                SkAnnotateNamedDestination((*pdfData.m_pages)[pdfData.m_currentPageIdx].m_canvas,
                                           targetPoint,
                                           pageName.get());
            ...
            Q_EMIT progress(static_cast<int>(static_cast<double>(itemIdx) / static_cast<double>(itemsCount) * 100.0));
        }
        ...
        Q_EMIT status(tr("Saving PDF..."));

        pdfData.save(m_fileName);

        Q_EMIT done(m_terminate);
...
void PdfRenderer::handleException(PdfAuxData &pdfData,
                                  const QString &msg)
{
#ifdef MD_PDF_TESTING
    m_isError = true;
...
void PdfRenderer::createPage(PdfAuxData &pdfData)
{
    ...
    create = [&create](PdfAuxData &pdfData) {
        ...
        page.m_canvas = page.m_recorder->beginRecording(bounds);
...
void PdfRenderer::drawHorizontalLine(PdfAuxData &pdfData)
{
    ...
    pdfData.drawLine(pdfData.m_layout.margins().m_left,
                     pdfData.m_layout.y(),
                     pdfData.m_layout.pageWidth() - pdfData.m_layout.margins().m_right,
                     pdfData.m_layout.y());
                         ...
                         RTLFlag *rtl)
{
    ...
    if (item && item->text().get()) {
        ...
        if (heightCalcOpt == CalcHeightOpt::Unknown && !item->label().isEmpty() && !where.first.isEmpty()) {
            SkPoint targetPoint =
                SkPoint::Make(pdfData.m_layout.borderStartX() + pdfData.m_layout.xIncrementDirection() * offset,
                              where.first.front().m_y - where.first.front().m_height);

            for (const auto &label : std::as_const(item->labelVariants())) {
                sk_sp<SkData> pageName(SkData::MakeWithCString(label.toUtf8().data()));
                SkAnnotateNamedDestination((*pdfData.m_pages)[where.first.front().m_pageIdx].m_canvas,
                                           targetPoint,
                                           pageName.get());
            }
        }
...
void PdfRenderer::initSubSupScript(MD::ItemWithOpts *item,
...
void PdfRenderer::deinitSubSupScript(MD::ItemWithOpts *item,
                                     PrevBaselineStateStack &state)
{
...
bool PdfRenderer::isTextOrOnlineAfter(MD::Block::Items::const_iterator it,
...
bool PdfRenderer::isTextOrOnlineBefore(MD::Block::Items::const_iterator it,
...
bool PdfRenderer::isNothingAfter(MD::Block::Items::const_iterator it,
                                 MD::Block::Items::const_iterator last)
{
    if (it != last) {
        it = std::next(it);
    ...
    if (!item->p()->isEmpty()) {
        for (auto it = item->p()->items().cbegin(), last = item->p()->items().cend(); it != last; ++it) {
            ...
            case MD::ItemType::Image: {
                auto prev =
                    (it != item->p()->items().begin() ? getPrevItem(std::prev(it), item->p()->items().begin(), last)
                                                      : nullptr);
                rects.append(drawImage(
                    pdfData,
...
inline bool isRightToLeft(const QVector<QPair<QString,
                                              bool>> &words)
{
...
inline bool isTheSameAlignment(ParagraphAlignment align,
                               bool rightToLeft)
{
    switch (align) {
...
void PdfRenderer::alignLine(PdfAuxData &pdfData,
                            const CustomWidth &cw)
{
    if (cw.alignment() != ParagraphAlignment::FillWidth) {
        pdfData.m_layout.moveXToBegin();
        const double delta = (isTheSameAlignment(cw.alignment(), pdfData.m_layout.isRightToLeft())
                                  ? 0.0
...
QVector<Word>::iterator PdfRenderer::drawWord(PdfAuxData &pdfData,
                                              QVector<Word>::iterator it,
                                              QVector<Word>::iterator last,
                                              double fontSize,
                                              double fontScale,
                        ...
                        bool useShaper)
{
    ...
    for (auto it = words.begin(), last = words.end(); it != last; ++it) {
        ...
        if (it->m_word == s_spaceString) {
            ...
            if (pdfData.m_layout.isRightToLeft() != it->m_rtl) {
                auto end = std::find_if(it, last, [&it](const auto &w) {
                    return (w.m_rtl != it->m_rtl && w.m_word != s_spaceString);
                });

                if (end != last && end->m_word != s_spaceString) {
                    ++end;
                }
...
double totalHeight(const QVector<WhereDrawn> &where,
                   bool withExtra = true)
{
    return std::accumulate(where.cbegin(),
                           where.cend(),
                           0.0,
                           [withExtra](const double &val, const WhereDrawn &cur) -> double {
                               return (val + cur.m_height + (withExtra ? cur.m_extraHeight : 0.0));
                           });
}

bool isRightToLeft(MD::Item *i)
{
    auto isTextRightToLeft = [](MD::Item *i) -> bool {
        return static_cast<MD::Text *>(i)->text().isRightToLeft();
    };
...
inline bool isLastInParagraph(MD::Block::Items::const_iterator it,
                              MD::Block::Items::const_iterator last)
{
    it = std::next(it);
...
QPair<QVector<WhereDrawn>,
      WhereDrawn>
PdfRenderer::drawParagraph(PdfAuxData &pdfData,
                           MD::Paragraph *item,
                           QSharedPointer<MD::Document> doc,
                           double offset,
                           bool withNewLine,
                           ...
                           ParagraphAlignment align)
{
    ...
    for (auto it = item->items().begin(), last = item->items().end(); it != last; ++it) {
        ...
        if (it + 1 != last && (it + 1)->get()->type() == MD::ItemType::FootnoteRef) {
            auto *ref = static_cast<MD::FootnoteRef *>((it + 1)->get());
        ...
        case MD::ItemType::FootnoteRef: {
            auto *ref = static_cast<MD::FootnoteRef *>(it->get());
    ...
    case CalcHeightOpt::Minimum: {
        ...
        r.append(
            {-1,
             0.0,
             cw.firstLineHeight(),
             ((withNewLine && !pdfData.m_firstOnPage) || (withNewLine && pdfData.m_drawFootnotes) ? lineHeight : 0.0)});
    ...
    for (auto it = item->items().begin(), last = item->items().end(); it != last; ++it) {
        ...
        case MD::ItemType::Link: {
            auto link = static_cast<MD::Link *>(it->get());
        ...
        case MD::ItemType::FootnoteRef: {
            ...
            if (fit != doc->footnotesMap().cend()) {
                ...
                SkPoint targetPoint = SkPoint::Make(rect.x(), rect.bottomY() - rect.height());
                sk_sp<SkData> refName(SkData::MakeWithCString(
                    (ref->id() + QStringLiteral("-%1").arg(QString::number(fcit.value()))).toUtf8().data()));
                SkAnnotateNamedDestination((*pdfData.m_pages)[pdfData.m_currentPainterIdx].m_canvas,
                                           targetPoint,
                                           refName.get());

                SkRect linkRect =
                    SkRect::MakeXYWH(rect.x(), rect.bottomY() - rect.height(), rect.width(), rect.height());
                sk_sp<SkData> footName(SkData::MakeWithCString(ref->id().toUtf8().data()));
                SkAnnotateLinkToDestination((*pdfData.m_pages)[pdfData.m_currentPainterIdx].m_canvas,
                                            linkRect,
                                            footName.get());
                          ...
                          PrevBaselineStateStack &previousBaseline)
{
    ...
    const auto latexRender = [&fontSize, &item](double inlineScale) -> std::unique_ptr<tex::TeXRender> {
        return std::unique_ptr<tex::TeXRender>(
            tex::LaTeX::parse(item->expr().toStdWString(),
                              0,
                              fontSize * (item->isInline() ? inlineScale : 1.0),
                              fontSize / 3.f * (item->isInline() ? inlineScale : 1.0),
                              tex::black));
    };
    ...
    render = latexRender(previousBaseline.currentScale());
    calculateSize();
    ...
    if (!item->isInline()) {
        ...
        double x = 0.0;
        double imgScale = 1.0;
        const double availableWidth = pdfData.m_layout.pageWidth()
        ...
        // y - is bottom of line.
        const double availableTotalWidth = pdfData.m_layout.pageWidth()
            - pdfData.m_layout.margins().m_left
            - pdfData.m_layout.margins().m_right
            - offset;
        ...
        if (size.width() - pdfData.m_layout.availableWidth() > 0.01) {
            ...
            if (prevItem) {
                ...
                case MD::ItemType::Text: {
                    auto t = static_cast<MD::Text *>(prevItem);
        ...
        if (draw) {
            ...
            cw.append({size.width() * imgScale,
                       subSupInit.wasAdded() ? lineInfo.first * imgScale : size.height() * imgScale,
                       false,
                       false,
                       isNextText,
                       "",
                       (descent + (subSupInit.wasAdded() ? lineInfo.second : 0.0)) * imgScale});
        }
    }
                                              ...
                                              RTLFlag *rtl)
{
    ...
    for (auto it = note->items().cbegin(), last = note->items().cend(); it != last; ++it) {
        ...
        case MD::ItemType::List: {
            ...
            ret.append(
                drawList(pdfData, list, doc, bulletWidth, footnoteOffset, heightCalcOpt, s_footnoteScale, false, rtl)
                    .first);
                       ...
                       bool scaleImagesToLineHeight)
{
    ...
    if (!img.isNull()) {
        ...
        return {r, pdfData.m_currentPainterIdx};
    } else {
...
QPair<QVector<WhereDrawn>,
      WhereDrawn>
PdfRenderer::drawCode(PdfAuxData &pdfData,
                      MD::Code *item,
                      QSharedPointer<MD::Document> doc,
                      double offset,
                      CalcHeightOpt heightCalcOpt,
...
QPair<QVector<WhereDrawn>,
      WhereDrawn>
PdfRenderer::drawBlockquote(PdfAuxData &pdfData,
                            MD::Blockquote *item,
                            QSharedPointer<MD::Document> doc,
                            double offset,
                            CalcHeightOpt heightCalcOpt,
                            ...
                            RTLFlag *rtl)
{
    ...
    if (heightCalcOpt == CalcHeightOpt::Unknown) {
        Q_EMIT status(tr("Drawing blockquote."));
    }
    ...
    for (auto it = item->items().cbegin(), last = item->items().cend(); it != last; ++it) {
        ...
        case MD::ItemType::Heading: {
            if (heightCalcOpt != CalcHeightOpt::Minimum) {
                const auto where = drawHeading(
                    pdfData,
                    static_cast<MD::Heading *>(it->get()),
                    doc,
                    offset + s_blockquoteBaseOffset,
                    (it + 1 != last
                         ? minNecessaryHeight(pdfData, *(it + 1), doc, offset + s_blockquoteBaseOffset, scale)
                         : 0.0),
                    heightCalcOpt,
                    scale,
                    true,
        ...
        case MD::ItemType::List: {
            ...
            const auto where = drawList(pdfData,
                                        list,
                                        doc,
                                        bulletWidth,
                                        offset + s_blockquoteBaseOffset,
...
QPair<QVector<WhereDrawn>,
      WhereDrawn>
PdfRenderer::drawList(PdfAuxData &pdfData,
                      MD::List *item,
                      QSharedPointer<MD::Document> doc,
                      int bulletWidth,
                      double offset,
    ...
    for (auto it = item->items().cbegin(), last = item->items().cend(); it != last; ++it) {
        if ((*it)->type() == MD::ItemType::ListItem) {
            const auto where = drawListItem(pdfData,
                                            static_cast<MD::ListItem *>(it->get()),
                                            doc,
                                            idx,
                                            prevListItemType,
                                            bulletWidth,
...
QPair<QVector<WhereDrawn>,
      WhereDrawn>
PdfRenderer::drawListItem(PdfAuxData &pdfData,
                          MD::ListItem *item,
                          QSharedPointer<MD::Document> doc,
                          int &idx,
                          ...
                          RTLFlag *rtl)
{
    ...
    for (auto it = item->items().cbegin(), last = item->items().cend(); it != last; ++it) {
        ...
        case MD::ItemType::List: {
            ...
            const auto where =
                drawList(pdfData, list, doc, maxListNumberWidth(list), offset, heightCalcOpt, scale, true, rtl);
...
int PdfRenderer::maxListNumberWidth(MD::List *list) const
{
    int counter = 0;
    bool first = true;
...
QPair<QVector<WhereDrawn>,
      WhereDrawn>
PdfRenderer::drawTable(PdfAuxData &pdfData,
                       MD::Table *item,
                       QSharedPointer<MD::Document> doc,
                       double offset,
                       CalcHeightOpt heightCalcOpt,
                       double scale)
{
    ...
    for (const auto &row : std::as_const(item->rows())) {
        ...
        const auto where = drawTableRow(row, pdfData, doc, item, offset, scale, columnWidth, rightToLeft, columnsCount);
...
QPair<QVector<WhereDrawn>,
      WhereDrawn>
PdfRenderer::drawTableRow(QSharedPointer<MD::TableRow> row,
                          PdfAuxData &pdfData,
                          QSharedPointer<MD::Document> doc,
                          MD::Table *table,
                          double offset,
                          ...
                          int columnsCount)
{
    ...
    for (const auto &c : std::as_const(row->cells())) {
        ...
        pdfData.m_layout.margins().m_left =
            (rightToLeftTable
                 ? pdfData.m_layout.pageWidth() - rightMargin - (columnWidth + s_tableMargin * 2.0) * (i + 1)
                 : leftMargin + i * (columnWidth + s_tableMargin * 2.0))
            + (pdfData.m_layout.isRightToLeft() ? -offset : offset)
            + s_tableMargin;
        pdfData.m_layout.margins().m_right =
            pdfData.m_layout.pageWidth() - pdfData.m_layout.margins().m_left - columnWidth;
...
inline ParagraphAlignment columnAlignmentToParagraphAlignment(MD::Table::Alignment align)
{
    switch (align) {
...
QPair<QVector<WhereDrawn>,
      WhereDrawn>
PdfRenderer::drawTableCell(QSharedPointer<MD::TableCell> cell,
                           PdfAuxData &pdfData,
                           QSharedPointer<MD::Document> doc,
                           ...
                           double scale)
{
    ...
    const auto ret = drawParagraph(pdfData,
                                   ...
                                   false,
                                   nullptr,
                                   columnAlignmentToParagraphAlignment(align));
...
void PdfRenderer::addFootnote(const QString &refId,
                              ...
                              QSharedPointer<MD::Document> doc)
{
    std::function<void(MD::Block * b, PdfAuxData & pdfData)> findFootnoteRefs;

    findFootnoteRefs = [&](MD::Block *b, PdfAuxData &pdfData) {
        for (auto it = b->items().cbegin(), last = b->items().cend(); it != last; ++it) {
            auto cb = dynamic_cast<MD::Block *>(it->get());
            ...
            if (cb) {
                ...
                switch ((*it)->type()) {
                case MD::ItemType::Heading:
                    findFootnoteRefs(static_cast<MD::Heading *>(it->get())->text().get(), pdfData);
                    break;

                case MD::ItemType::Table: {
                    auto t = static_cast<MD::Table *>(it->get());
    ...
    if (std::find_if(m_footnotes.cbegin(),
        ...
        tmpData.m_layout.m_coords = {{pdfData.m_layout.margins().m_left,
                                      pdfData.m_layout.margins().m_right,
                                       ...
                                       double scale)
{
    ...
    case MD::ItemType::Paragraph: {
        ret = drawParagraph(tmp,
                            static_cast<MD::Paragraph *>(item.get()),
                            doc,
                            offset,
                            true,
    ...
    case MD::ItemType::List: {
        auto *list = static_cast<MD::List *>(item.get());
        const auto bulletWidth = maxListNumberWidth(list);
