...
function getTags(pInnerHTML) {
    const html = pInnerHTML.toLowerCase();
    let ret = [];
    for (const key of Object.keys(tagClassNameMap)) {
        const i = html.indexOf(key);
        if (i != -1) {
            ret.push(pInnerHTML.substring(i, i + key.length));
        }
    }
    return ret;
}
            ...
            if (e instanceof HTMLQuoteElement) {
                ...
                if (p instanceof HTMLParagraphElement) {
                    ...
                    if (tags.length) {
                        ...
                        e.prepend(createImageParagraph(lowerTag));
                        e.classList.add('markdown-alert', getTagClassname(lowerTag));
                    }
                }
            }
        });
